...
export function CobrosPanel({ clientes, pedidos, devoluciones, onBack }) {
  ...
  async function handleEnviar(cliente) {
    ...
    const msg = `Hola ${cliente.nombre}! 👋 Te escribo para recordarte que tenés un saldo pendiente de ${formatCurrency(cliente.saldo)} por tu pedido del ${fechaUlt}. Cuando puedas lo coordinamos. ¡Gracias!`;

    setEnviando(cliente.id);
    try {
      await registrarComunicacion(cliente.id, 'recordatorio', msg);
    } catch (_) {}
            ...
            <motion.div key={c.id} className="card" {...listItem(i)}>
              ...
              {c.contacto && (
                ...
                >
                  ...
                  {enviando === c.id ? 'Enviando...' : 'Enviar recordatorio'}
                </button>
              )}
              {!c.contacto && (
